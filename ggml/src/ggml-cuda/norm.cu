...
static __global__ void rms_norm_f32(const float * x,
    ...
    for (int col = tid; col < ncols; col += block_size) {
        if constexpr (do_multiply && do_add) {
            ...
            dst[col] = scale * x[col] * post_scale;
        }
    }
}
...
// RMS_NORM → SCALE（無偏移）合成一次：dst = (rsqrt 係數 * x) * s，兩次捨入與分開算完全一致
void ggml_cuda_op_rms_norm_scale(ggml_backend_cuda_context & ctx, ggml_tensor * dst, ggml_tensor * scale_tensor) {
    const ggml_tensor * src0 = dst->src[0];
    float eps; memcpy(&eps, dst->op_params, sizeof(float));
    float s;   memcpy(&s, scale_tensor->op_params, sizeof(float));
    GGML_ASSERT(src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32 && scale_tensor->type == GGML_TYPE_F32);
    GGML_TENSOR_UNARY_OP_LOCALS;
    const size_t ts0 = ggml_type_size(src0->type);
    GGML_ASSERT(nb00 == ts0);
    rms_norm_f32_cuda((const float *) src0->data, (float *) scale_tensor->data, ne00, ne01, ne02, ne03,
                      nb01 / ts0, nb02 / ts0, nb03 / ts0, eps, ctx.stream(), s);
}
